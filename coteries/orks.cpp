...
std::shared_ptr<Effet> Orks::AjouterEffetUniversite(GenHistoire* genHist, shared_ptr<Evt> evt, QString go_to_effet_suivant )
{
    ...
    // blessure dans la fosse
    {
        ...
        effet->AjouterChangeurDeCarac(PbSante::DEFIGURE, "1");
        effet->AjouterAjouteurACarac(Combat::C_CAP_COMBAT, "1"); // meilleur combattant
        effet->AjouterChangeurDeCarac(Trait::GetNomTrait(eTrait::beau), ""); // le joueur perd son éventuelle beauté...
    ...
    // fou de la vitesse
    {
        ...
        effet->AjouterAjouteurACarac(Vehicule::C_CONDUITE_ROUTE, "2");
        GenVieHumain::TransformerEffetEnEffetMoisDeVie(effet, false);
        effet->AjouterAjouteurACarac(Metier::CONDUCTEUR, "2");
    ...
    // pilote d'avion
    {
        shared_ptr<Effet> effet = genHist->AjouterEffetNarration(
                    ...
                    "", evt);
        effet->AjouterAjouteurACarac(Vehicule::C_CONDUITE_AVION, "1");
        effet->AjouterAjouteurACarac(Metier::PILOTE, "1");
        GenVieHumain::TransformerEffetEnEffetMoisDeVie(effet, false);
    ...
    // soulé à la bière
    {
        ...
        if ( proba <= 0.3) {
            ...
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(faible), "1");
        }
        else if (proba > 0.7 )
        {
            effet->m_Texte += "\nVous gagnez 'résistant'.";
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(resistant), "1");
        }
        proba = Aleatoire::GetAl()->Entre0Et1();
        if ( proba <= 0.4 ) {
            effet->m_Texte += "\nToutes vos angoisses profondes fondent définitivement.";
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(angoisse), "");
        }
        proba = Aleatoire::GetAl()->Entre0Et1();
        if ( proba <= 0.5 ) {
            effet->m_Texte += "\nDurablement affecté par la boisson vous devenez alcoolique.";
            effet->AjouterChangeurDeCarac(PbSante::ALCOOLIQUE, "1");
        }
        proba = Aleatoire::GetAl()->Entre0Et1();
        if ( proba <= 0.2 ) {
            effet->m_Texte += "\nL'alcool vous a salement endommagé le cerveau. Vous devenez 'simple'";
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(simple), "1");
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(intelligent), "");
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(intellectuel), "");
        }
        proba = Aleatoire::GetAl()->Entre0Et1();
        if ( proba <= 0.2 ) {
            ...
            effet->AjouterChangeurDeCarac(Trait::GetNomTrait(franc), "1");
        }
    ...
    // formation mékano
    {
        ...
        effet->AjouterAjouteurACarac(Ingenieur::C_MECANIQUE, "1"); // meilleur Mékano
